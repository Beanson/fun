                ...
                for($i=0;$i<count($data);$i++) {
                    for ($j=$i+1; $j < count($data); $j++) {
                        if ($data[$i]['name'] == $data[$j]['name']) {
                            //同一天上的课，判断为同一节课
                            if ($data[$i]['week'] == $data[$j]['week']) {
                                $start_min = min((int)$data[$i]['startSection'], (int)$data[$j]['startSection']);
                                $data[$i]['startSection'] = $start_min;
                                $data[$j]['startSection'] = $start_min;

                                $end_max = max((int)$data[$i]['endSection'], (int)$data[$j]['endSection']);
                                $data[$i]['endSection'] = $end_max;
                                $data[$j]['endSection'] = $end_max;
...
//                            else {}
    ...
    public function  getGeneralInformation(){
        $requestUrl = $this->getRequestUrl($this->username, 3);
        $this->saveContent($requestUrl);
        $pageContent = $this->getContent();
        //$pageContent = iconv("gbk", "utf-8", $pageContent);
        return $pageContent;
